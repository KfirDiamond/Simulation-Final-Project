...
def compute_counts_machine_types(num_stations, sim_time, capacities, mu, num_jobs, num_simulations, machine_types_probs):
    stations_res_list = {}
    ...
    for ind in tqdm(range(num_simulations)):
        ...
        sim_res, count_num_jobs_complete = single_sim_rate_arrival_throughtput(mu, sim_time, capacities, num_stations, num_jobs, machine_types_probs)
...
def single_sim_rate_arrival_throughtput(mu, sim_time, capacities, num_stations, num_jobs, machine_types_probs):

    sim_time += 1
    sim_instance = Queue_multi_stations(mu, sim_time, capacities,  num_stations, num_jobs, machine_types_probs)
    sim_instance.run()
    ...
    return  (resultDictionary, sim_instance.count_num_jobs_complete)
...
class Queue_multi_stations:
    ...
    def __init__(self,  mu, sim_time, capacities, num_stations, num_jobs, machine_types_probs):
        ...
        self.count_num_jobs_complete = 0
        self.machine_types_probs = 1-machine_types_probs
        ...
        for station in range(1, num_stations + 1):
            ...
            self.num_cust_durations[station] = np.zeros(num_jobs+1)  ## the time duration of each each state (state= number of cusotmers in the system)
    ...
    def service(self, station, customer):
        ...
        if station < self.num_stations:
            ...
            if np.random.rand() < self.machine_types_probs.prod():
                self.count_num_jobs_complete += 1
