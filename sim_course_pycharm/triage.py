...
class Queue_triage:
    ...
    def run(self):
        print('Starting simulating triage...')
        for type_ in range(2):

            self.env.process(self.customer_arrivals(type_))  ## Initializing a process
...
#                                               ignore_index=True)
        ...
        if station == 2:
            ...
            self.env.process(self.service(customer, customer.type))

    #########################################################
    ################# Arrival block #########################
    #########################################################

    def customer_arrivals(self, customer_type):


        while True:


            yield self.env.timeout(np.random.exponential(1 / self.arrival_rates[customer_type]))
    ...
    # def get_average_waiting_time_triage(self):
    #     return (self.df_waiting_times[0]['WaitingTime'].mean(), self.df_waiting_times[1]['WaitingTime'].mean()                           ,self.df_waiting_times[2]['WaitingTime'].mean())
    
    def get_average_waiting_time_triage(self):
        return (np.array(self.waiting_times[0]).mean(), np.array(self.waiting_times[1]).mean(),                   np.array(self.waiting_times[2]).mean())
    
    
# arrival_rates = np.array([2, 2])
# service_rates = np.array([3, 4, 10])



# sim_time = 7000
# queue_triage = Queue_triage(arrival_rates,service_rates,   sim_time)
# queue_triage.run()
