...
class Queue_single_station_2_servers:

    def __init__(self, lamda, mu, sim_time):

        self.env = simpy.Environment()  # initializing simpy enviroment
        self.server = simpy.Resource(self.env, capacity=2)  # Defining a resource with capacity 1
    ...
    def service(self, customer):
        ...
        with self.server.request() as req:
            ...
            self.df_waiting_times = pd.concat([self.df_waiting_times, pd.DataFrame([new_waiting_row])],
                                              ignore_index=True)
...
sim_time = 1000
queue_two_stations = Queue_single_station_2_servers(lamda, mu, sim_time)
